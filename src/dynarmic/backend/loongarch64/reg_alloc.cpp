...
namespace Dynarmic::Backend::LoongArch64 {
    ...
    void RegAlloc::EmitVerboseDebuggingOutput() {
        code.add_imm(code.s0, code.zero, mcl::bit_cast<u64>(&PrintVerboseDebuggingOutputLine),
                     Xscratch0);  // Non-volatile register
...
//    ASSERT(value.GetType() != IR::Type::U1);
        if constexpr (kind == HostLoc::Kind::Gpr) {
            auto scratchGpr = getScratchGpr();

            code.add_imm(scratchGpr, code.zero, value.GetImmediateAsU64(), Xscratch0);

            return scratchGpr.getIdx();
        } else if constexpr (kind == HostLoc::Kind::Fpr) {
            auto scratchFpr = getScratchFpr();

            code.add_imm(Xscratch0, code.zero, value.GetImmediateAsU64(), Xscratch1);
            code.vinsgr2vr_d(scratchFpr, Xscratch0, 0);

            return scratchFpr.getIdx();
...
//        code.MSR(Xbyak_loongarch64::SystemReg::NZCV, Xscratch0);
    ...
    Xbyak_loongarch64::VReg RegAlloc::getScratchFpr() {
        ...
        return Xbyak_loongarch64::VReg{new_location_index};
    }

    Xbyak_loongarch64::XReg RegAlloc::getScratchGpr() {
        ...
        return Xbyak_loongarch64::XReg{new_location_index};
    }
        ...
        if constexpr (required_kind == HostLoc::Kind::Gpr) {
            ...
            switch (current_location->kind) {
                ...
                case HostLoc::Kind::Fpr:
                    code.vpickve2gr_d(Xbyak_loongarch64::XReg{new_location_index},
                                      Xbyak_loongarch64::VReg{current_location->index}, 0);
                    // ASSERT size fits
                    break;
                case HostLoc::Kind::Spill:
                    code.ld_d(Xbyak_loongarch64::XReg{new_location_index}, code.sp,
...
//            code.MRS(Xbyak_loongarch64::XReg{new_location_index}, Xbyak_loongarch64::SystemReg::NZCV);
            ...
            switch (current_location->kind) {
                case HostLoc::Kind::Gpr:
                    code.vinsgr2vr_d(Xbyak_loongarch64::VReg{new_location_index},
                                     Xbyak_loongarch64::XReg{current_location->index}, 0);
                    break;
                case HostLoc::Kind::Fpr:
                    ASSERT_FALSE("Logic error");
                    break;
    ...
    unsigned int RegAlloc::RealizeWriteImpl(const IR::Inst *value) {
        ...
        ASSERT_MSG(!ValueLocation(value),
                   Dynarmic::IR::GetNameOf((IR::Type)op) + " " + std::to_string( value->GetName()) + "\r\n");
    ...
    RegAlloc::RealizeReadWriteImpl<HostLoc::Kind::Gpr>(const IR::Value &read_value, const IR::Inst *write_value);

    template unsigned int
    RegAlloc::RealizeReadWriteImpl<HostLoc::Kind::Fpr>(const IR::Value &read_value, const IR::Inst *write_value);

    template unsigned int
    RegAlloc::RealizeReadWriteImpl<HostLoc::Kind::Flags>(const IR::Value &read_value, const IR::Inst *write_value);
    ...
    void RegAlloc::LoadCopyInto(const IR::Value &value, Xbyak_loongarch64::VReg reg) {
        ...
        switch (current_location->kind) {
            case HostLoc::Kind::Gpr:
                ...
                break;
            case HostLoc::Kind::Fpr:
                code.vor_v(reg, Xbyak_loongarch64::VReg{current_location->index},
                           Xbyak_loongarch64::VReg{current_location->index});
                break;
            case HostLoc::Kind::Spill:
