...
//            if (code.HasHostFeature(HostFeature::AVX)) {
    ...
    struct DefaultIndexer {
        ...
        std::tuple<T, T, T>
        operator()(size_t i, const VectorArray<T> &a, const VectorArray<T> &b, const VectorArray<T> &c) {
    ...
    template<size_t fsize>
    void DenormalsAreZero(BlockOfCode &code, FP::FPCR fpcr, std::initializer_list<Xbyak_loongarch64::VReg> to_daz,
                          Xbyak_loongarch64::VReg tmp) {
...
//        code.vxor_v(result, result, result);
    ...
    static void EmitFPVectorMinMax(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        if (ctx.FPCR(fpcr_controlled).DN()) {
            ...
            auto mask = ctx.reg_alloc.AllocScratchFpr();
            auto eq = ctx.reg_alloc.AllocScratchFpr();
            auto nan_mask = ctx.reg_alloc.AllocScratchFpr();
...
//                if (code.HasHostFeature(HostFeature::AVX)) {
        ...
        EmitThreeOpVectorOperation<fsize, DefaultIndexer>(
                code, ctx, inst, [&](Xbyak_loongarch64::VReg result, Xbyak_loongarch64::VReg xmm_b) {
                    auto mask = ctx.reg_alloc.AllocScratchFpr();
                    auto eq = ctx.reg_alloc.AllocScratchFpr();
                    auto tmp_xmm_b = ctx.reg_alloc.AllocScratchFpr();
                    if (ctx.FPCR(fpcr_controlled).FZ()) {
                        const Xbyak_loongarch64::VReg prev_xmm_b = xmm_b;
                        // this is safe for that xmm_b unused after this lambda has done.
                        xmm_b = tmp_xmm_b;
...
//                    if (code.HasHostFeature(HostFeature::AVX)) {
    ...
    static void EmitFPVectorMinMaxNumeric(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        RegAlloc::Realize(xmm_a, xmm_b, result);
        auto intermediate_result = ctx.reg_alloc.AllocScratchFpr();
        auto tmp1 = ctx.reg_alloc.AllocScratchFpr();
        auto tmp2 = ctx.reg_alloc.AllocScratchFpr();
...
//        if (code.HasHostFeature(HostFeature::AVX)) {
    ...
    void EmitIR<IR::Opcode::FPVectorEqual16>(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        EmitThreeOpFallback(code, ctx, inst,
                            [](VectorArray<u16> &result, const VectorArray<u16> &op1, const VectorArray<u16> &op2,
        ...
        static const auto lut = Common::GenerateLookupTableFromList(
                []<typename I>(I) {
                    return std::pair{
                            ...
                            Common::FptrCast(
                                    [](VectorArray<u32> &output, const VectorArray<u16> &input, FP::FPCR fpcr,
                                       FP::FPSR &fpsr) {
...
//        });
    ...
    void EmitFPVectorMulAdd(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        const auto fallback_fn = [](VectorArray<FPT> &result, const VectorArray<FPT> &addend,
                                    const VectorArray<FPT> &op1, const VectorArray<FPT> &op2, FP::FPCR fpcr,
                                    FP::FPSR &fpsr) {
            for (size_t i = 0; i < result.size(); i++) {
...
//            code.FADDP(Qresult->toD(), V0.D2());
    ...
    static void EmitRecipEstimate(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        EmitTwoOpFallback(code, ctx, inst,
                          [](VectorArray<FPT> &result, const VectorArray<FPT> &operand, FP::FPCR fpcr, FP::FPSR &fpsr) {
                              for (size_t i = 0; i < result.size(); i++) {
    ...
    static void EmitRecipStepFused(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        const auto fallback_fn = [](VectorArray<FPT> &result, const VectorArray<FPT> &op1, const VectorArray<FPT> &op2,
                                    FP::FPCR fpcr, FP::FPSR &fpsr) {
            for (size_t i = 0; i < result.size(); i++) {
        ...
        static const auto lut = Common::GenerateLookupTableFromList(
                []<typename I>(I) {
                    ...
                    return std::pair{
                            mp::lower_to_tuple_v<I>,
                            Common::FptrCast(
                                    [](VectorArray<FPT> &output, const VectorArray<FPT> &input, FP::FPCR fpcr,
                                       FP::FPSR &fpsr) {
                                        constexpr FP::RoundingMode rounding_mode = mp::get<0, I>::value;
                                        constexpr bool exact = mp::get<1, I>::value;

                                        for (size_t i = 0; i < output.size(); ++i) {
                                            output[i] = static_cast<FPT>(FP::FPRoundInt<FPT>(input[i], fpcr,
                                                                                             rounding_mode, exact,
                                                                                             fpsr));
                                        }
                                    })};
    ...
    static void EmitRSqrtEstimate(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
        ...
        const auto fallback_fn = [](VectorArray<FPT> &result, const VectorArray<FPT> &operand, FP::FPCR fpcr,
                                    FP::FPSR &fpsr) {
            for (size_t i = 0; i < result.size(); i++) {
        ...
        static const auto lut = Common::GenerateLookupTableFromList(
                []<typename I>(I) {
                    return std::pair{
                            mp::lower_to_tuple_v<I>,
                            Common::FptrCast(
                                    [](VectorArray<u16> &output, const VectorArray<u32> &input, FP::FPCR fpcr,
                                       FP::FPSR &fpsr) {
                                        constexpr FP::RoundingMode rounding_mode = mp::get<0, I>::value;

                                        for (size_t i = 0; i < output.size(); ++i) {
                                            if (i < input.size()) {
                                                output[i] = FP::FPConvert<u16, u32>(input[i], fpcr, rounding_mode,
                                                                                    fpsr);
                                            } else {
                                                output[i] = 0;
                                            }
                            ...
                            Common::FptrCast(
                                    [](VectorArray<FPT> &output, const VectorArray<FPT> &input, FP::FPCR fpcr,
                                       FP::FPSR &fpsr) {
                                        ...
                                        for (size_t i = 0; i < output.size(); ++i) {
                                            output[i] = static_cast<FPT>(FP::FPToFixed<FPT>(fsize, input[i], fbits,
                                                                                            unsigned_, fpcr,
                                                                                            rounding_mode, fpsr));
                                        }
                                    })};
