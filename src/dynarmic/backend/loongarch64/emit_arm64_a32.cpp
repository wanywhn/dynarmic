...
void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::Terminal terminal, IR::LocationDescriptor initial_location, bool is_single_step);

void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator&, EmitContext&, IR::Term::Interpret, IR::LocationDescriptor, bool) {
    ASSERT_FALSE("Interpret should never be emitted.");
}

void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::ReturnToDispatch, IR::LocationDescriptor, bool) {
    EmitRelocation(code, ctx, LinkTarget::ReturnToDispatcher);
}

static void EmitSetUpperLocationDescriptor(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::LocationDescriptor new_location, IR::LocationDescriptor old_location) {
...
void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::LinkBlock terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
...
void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::LinkBlockFast terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
    EmitSetUpperLocationDescriptor(code, ctx, terminal.next, initial_location);
...
void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::PopRSBHint, IR::LocationDescriptor, bool is_single_step) {
...
void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::FastDispatchHint, IR::LocationDescriptor, bool) {
    EmitRelocation(code, ctx, LinkTarget::ReturnToDispatcher);

    // TODO: Implement FastDispatchHint optimization
}

void EmitA32Terminal(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Term::If terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
...
template<>
void EmitIR<IR::Opcode::A32GetExtendedRegister32>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
...
template<>
void EmitIR<IR::Opcode::A32GetExtendedRegister64>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
...
template<>
void EmitIR<IR::Opcode::A32SetExtendedRegister32>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
    ASSERT(A32::IsSingleExtReg(reg));
    const size_t index = static_cast<size_t>(reg) - static_cast<size_t>(A32::ExtReg::S0);
...
template<>
void EmitIR<IR::Opcode::A32SetExtendedRegister64>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
    ASSERT(A32::IsDoubleExtReg(reg));
    const size_t index = static_cast<size_t>(reg) - static_cast<size_t>(A32::ExtReg::D0);
...
void EmitIR<IR::Opcode::A32SetCpsr>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    ...
    // NZCV, Q flags
    code.andi(Wscratch0, Wcpsr, 0xF0000000);
    ...
    code.andi(Wscratch1, Wcpsr, Wscratch1);
    ...
    // IT state
    code.andi(Wscratch0, Wcpsr, 0xFC00);
...
template<>
void EmitIR<IR::Opcode::A32SetCpsrNZCVRaw>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    auto Wnzcv = ctx.reg_alloc.ReadW(args[0]);
    RegAlloc::Realize(Wnzcv);
...
template<>
void EmitIR<IR::Opcode::A32SetGEFlagsCompressed>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void EmitIR<IR::Opcode::A32BXWritePC>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    ...
    if (args[0].IsImmediate()) {
        ...
        code.add_d(Xscratch0, (u64{new_upper} << 32) | (new_pc & mask), code.zero);
...
template<>
void EmitIR<IR::Opcode::A32UpdateUpperLocationDescriptor>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst*) {
...
template<>
void EmitIR<IR::Opcode::A32CallSupervisor>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
template<>
void EmitIR<IR::Opcode::A32ExceptionRaised>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    ctx.reg_alloc.PrepareForCall();
...
template<>
void EmitIR<IR::Opcode::A32DataSynchronizationBarrier>(Xbyak_loongarch64::CodeGenerator& code, EmitContext&, IR::Inst*) {
...
template<>
void EmitIR<IR::Opcode::A32InstructionSynchronizationBarrier>(Xbyak_loongarch64::CodeGenerator& code, EmitContext& ctx, IR::Inst*) {
