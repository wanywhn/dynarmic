...
namespace Dynarmic::Backend::LoongArch64 {
    ...
    AddressSpace::AddressSpace(size_t code_cache_size, JitStateInfo jsi)
            ...
            //        , mem(code_cache_size)
            , code(code_cache_size, jsi), fastmem_manager(exception_handler) {
        // limited by jump
...
//    mem.invalidate(reinterpret_cast<u32*>(block_info.entry_point), block_info.size);
    ...
    static void LinkBlockLinks(LoongArch64::BlockOfCode &c, const CodePtr entry_point, const CodePtr target_ptr,
                               const std::vector<BlockRelocation> &block_relocations_list, void *return_to_dispatcher) {
...
//    using namespace oaknut;
        ...
        for (auto [target_descriptor, list]: block_info.block_relocations) {
            ...
            LinkBlockLinks(code, block_info.entry_point, Get(target_descriptor), list,
    ...
    void AddressSpace::RelinkForDescriptor(IR::LocationDescriptor target_descriptor, CodePtr target_ptr) {
        for (auto code_ptr: block_references[target_descriptor]) {
            if (auto block_iter = block_infos.find(code_ptr); block_iter != block_infos.end()) {
                ...
                if (auto relocation_iter = block_info.block_relocations.find(target_descriptor); relocation_iter !=
                                                                                                 block_info.block_relocations.end()) {
                    LinkBlockLinks(code, block_info.entry_point, target_ptr, relocation_iter->second,
                                   prelude_info.return_to_dispatcher);
                }
                // FIXME
//            mem.invalidate(reinterpret_cast<u32*>(block_info.entry_point), block_info.size);
