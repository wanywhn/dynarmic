...
// FIXME see x86
    ...
    static void EmitToFixed(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst) {
                    ...
                    return std::pair{
                            ...
                            Common::FptrCast(
                                    [](u64 input, FP::FPSR &fpsr, FP::FPCR fpcr) {
                                        ...
                                        return FP::FPToFixed<FPT>(bitsize_to, static_cast<FPT>(input), fbits, unsigned_,
                                                                  fpcr, rounding_mode, fpsr);
                                    })};
                },
        ...
        code.add_imm(Xscratch0, code.zero, mcl::bit_cast<u64>(lut.at(std::make_tuple(fbits, rounding_mode))),
                     Xscratch2);
...
//    EmitFourOp<64>(code, ctx, inst, [&](auto& Dresult, auto& Da, auto& D1, auto& D2) { code.fmadd_d(Dresult, D1, D2, Da); });
    ...
    static void EmitFPRound(BlockOfCode &code, EmitContext &ctx, IR::Inst *inst, size_t fsize) {
                    ...
                    return std::pair{
                            ...
                            Common::FptrCast(
                                    [](u64 input, FP::FPSR &fpsr, FP::FPCR fpcr) {
                                        ...
                                        return FP::FPRoundInt<InputSize>(static_cast<InputSize>(input), fpcr,
                                                                         rounding_mode, exact, fpsr);
                                    })};
                },
        ...
        code.add_imm(Xscratch0, code.zero, mcl::bit_cast<u64>(lut.at(std::make_tuple(fsize, rounding_mode, exact))),
                     Xscratch2);
        code.jirl(code.ra, Xscratch0, 0);
        code.movgr2fr_d(Rto, code.a0);
        ABI_PopRegisters(code, ABI_CALLER_SAVE & ~ToRegList(*Rto) & ~ToRegList(*Vfrom), 0);
