...
namespace Xbyak = Xbyak_loongarch64;
...
EmitLoongArch64::EmitLoongArch64(BlockOfCode& code)
        : code(code) {
    exception_handler.Register(code);
}

EmitLoongArch64::~EmitLoongArch64() = default;

std::optional<EmitLoongArch64::BlockDescriptor> EmitLoongArch64::GetBasicBlock(IR::LocationDescriptor descriptor) const {
...
void EmitLoongArch64::EmitVoid(EmitContext&, IR::Inst*) {
}

void EmitLoongArch64::EmitIdentity(EmitContext& ctx, IR::Inst* inst) {
...
void EmitLoongArch64::EmitBreakpoint(EmitContext&, IR::Inst*) {
    code.int3();
}

void EmitLoongArch64::EmitCallHostFunction(EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void EmitLoongArch64::PushRSBHelper(Xbyak::Reg64 loc_desc_reg, Xbyak::Reg64 index_reg, IR::LocationDescriptor target) {
    using namespace Xbyak::util;
...
void EmitLoongArch64::EmitVerboseDebuggingOutput(RegAlloc& reg_alloc) {
...
void EmitLoongArch64::EmitPushRSB(EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void EmitLoongArch64::EmitGetCarryFromOp(EmitContext& ctx, IR::Inst* inst) {
    ctx.reg_alloc.RegisterPseudoOperation(inst);
}

void EmitLoongArch64::EmitGetOverflowFromOp(EmitContext& ctx, IR::Inst* inst) {
    ctx.reg_alloc.RegisterPseudoOperation(inst);
}

void EmitLoongArch64::EmitGetGEFromOp(EmitContext& ctx, IR::Inst* inst) {
    ctx.reg_alloc.RegisterPseudoOperation(inst);
}

void EmitLoongArch64::EmitGetUpperFromOp(EmitContext& ctx, IR::Inst* inst) {
    ctx.reg_alloc.RegisterPseudoOperation(inst);
}

void EmitLoongArch64::EmitGetLowerFromOp(EmitContext& ctx, IR::Inst* inst) {
    ctx.reg_alloc.RegisterPseudoOperation(inst);
}

void EmitLoongArch64::EmitGetNZFromOp(EmitContext& ctx, IR::Inst* inst) {
...
void EmitLoongArch64::EmitGetNZCVFromOp(EmitContext& ctx, IR::Inst* inst) {
    if (ctx.reg_alloc.IsValueLive(inst)) {
        ctx.reg_alloc.RegisterPseudoOperation(inst);
        return;
    }
...
void EmitLoongArch64::EmitGetCFlagFromNZCV(EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void EmitLoongArch64::EmitNZCVFromPackedFlags(EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void EmitLoongArch64::EmitAddCycles(size_t cycles) {
...
Xbyak::Label EmitLoongArch64::EmitCond(IR::Cond cond) {
    Xbyak::Label pass;
...
EmitLoongArch64::BlockDescriptor EmitLoongArch64::RegisterBlock(const IR::LocationDescriptor& descriptor, CodePtr entrypoint, size_t size) {
...
void EmitLoongArch64::EmitTerminal(IR::Terminal terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
...
void EmitLoongArch64::Patch(const IR::LocationDescriptor& target_desc, CodePtr target_code_ptr) {
...
void EmitLoongArch64::Unpatch(const IR::LocationDescriptor& target_desc) {
    if (patch_information.count(target_desc)) {
        Patch(target_desc, nullptr);
    }
}

void EmitLoongArch64::ClearCache() {
...
void EmitLoongArch64::InvalidateBasicBlocks(const tsl::robin_set<IR::LocationDescriptor>& locations) {
    code.EnableWriting();
    SCOPE_EXIT {
        code.DisableWriting();
    };
