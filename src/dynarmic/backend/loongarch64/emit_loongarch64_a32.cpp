...
#include "dynarmic/backend/loongarch64/emit_loongarch64_a32.h"
...
#include "dynarmic/backend/loongarch64/nzcv_util.h"
...
A32EmitLoongArch64::A32EmitLoongArch64(BlockOfCode& code, A32::UserConfig conf, A32::Jit* jit_interface)
        : EmitLoongArch64(code), conf(std::move(conf)), jit_interface(jit_interface) {
    GenFastmemFallbacks();
    GenTerminalHandlers();
...
A32EmitLoongArch64::~A32EmitLoongArch64() = default;

A32EmitLoongArch64::BlockDescriptor A32EmitLoongArch64::Emit(IR::Block& block) {
...
#define OPCODE(name, type, ...)            \
    case IR::Opcode::name:                 \
        A32EmitLoongArch64::Emit##name(ctx, inst); \
        break;
#define A32OPC(name, type, ...)               \
    case IR::Opcode::A32##name:               \
        A32EmitLoongArch64::EmitA32##name(ctx, inst); \
        break;
#define A64OPC(...)
...
#undef A64OPC
    ...
    EmitLoongArch64::EmitTerminal(block.GetTerminal(), ctx.Location().SetSingleStepping(false), ctx.IsSingleStep());
    code.int3();
...
void A32EmitLoongArch64::ClearCache() {
    EmitLoongArch64::ClearCache();
    block_ranges.ClearCache();
    ClearFastDispatchTable();
    fastmem_patch_info.clear();
}

void A32EmitLoongArch64::InvalidateCacheRanges(const boost::icl::interval_set<u32>& ranges) {
    InvalidateBasicBlocks(block_ranges.InvalidateRanges(ranges));
}

void A32EmitLoongArch64::EmitCondPrelude(const A32EmitContext& ctx) {
...
void A32EmitLoongArch64::ClearFastDispatchTable() {
    if (conf.HasOptimization(OptimizationFlag::FastDispatch)) {
        fast_dispatch_table.fill({});
    }
}

void A32EmitLoongArch64::GenTerminalHandlers() {
...
void A32EmitLoongArch64::EmitA32SetCheckBit(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32GetRegister(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32GetExtendedRegister32(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32GetExtendedRegister64(A32EmitContext& ctx, IR::Inst* inst) {
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
...
void A32EmitLoongArch64::EmitA32GetVector(A32EmitContext& ctx, IR::Inst* inst) {
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
...
void A32EmitLoongArch64::EmitA32SetRegister(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    const A32::Reg reg = inst->GetArg(0).GetA32RegRef();
...
void A32EmitLoongArch64::EmitA32SetExtendedRegister32(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
    ASSERT(A32::IsSingleExtReg(reg));
...
void A32EmitLoongArch64::EmitA32SetExtendedRegister64(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
    ASSERT(A32::IsDoubleExtReg(reg));
...
void A32EmitLoongArch64::EmitA32SetVector(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    const A32::ExtReg reg = inst->GetArg(0).GetA32ExtRegRef();
    ASSERT(A32::IsDoubleExtReg(reg) || A32::IsQuadExtReg(reg));
...
void A32EmitLoongArch64::EmitA32GetCpsr(A32EmitContext& ctx, IR::Inst* inst) {
    const Xbyak::Reg32 result = ctx.reg_alloc.ScratchGpr().cvt32();
...
void A32EmitLoongArch64::EmitA32SetCpsr(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32SetCpsrNZCV(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32SetCpsrNZCVRaw(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32SetCpsrNZCVQ(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    if (args[0].IsImmediate()) {
        const u32 imm = args[0].GetImmediateU32();
...
void A32EmitLoongArch64::EmitA32SetCpsrNZ(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32SetCpsrNZC(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32GetCFlag(A32EmitContext& ctx, IR::Inst* inst) {
    EmitGetFlag(code, ctx, inst, NZCV::x64_c_flag_bit);
}

void A32EmitLoongArch64::EmitA32OrQFlag(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    if (args[0].IsImmediate()) {
...
void A32EmitLoongArch64::EmitA32GetGEFlags(A32EmitContext& ctx, IR::Inst* inst) {
    const Xbyak::Xmm result = ctx.reg_alloc.ScratchXmm();
    code.movd(result, dword[r15 + offsetof(A32JitState, cpsr_ge)]);
    ctx.reg_alloc.DefineValue(inst, result);
}

void A32EmitLoongArch64::EmitA32SetGEFlags(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32SetGEFlagsCompressed(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    if (args[0].IsImmediate()) {
        const u32 imm = args[0].GetImmediateU32();
        u32 ge = 0;
...
void A32EmitLoongArch64::EmitA32DataSynchronizationBarrier(A32EmitContext&, IR::Inst*) {
    code.mfence();
    code.lfence();
}

void A32EmitLoongArch64::EmitA32DataMemoryBarrier(A32EmitContext&, IR::Inst*) {
    code.mfence();
}

void A32EmitLoongArch64::EmitA32InstructionSynchronizationBarrier(A32EmitContext& ctx, IR::Inst*) {
    if (!conf.hook_isb) {
        return;
    }
...
void A32EmitLoongArch64::EmitA32BXWritePC(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    auto& arg = args[0];
...
void A32EmitLoongArch64::EmitA32UpdateUpperLocationDescriptor(A32EmitContext& ctx, IR::Inst*) {
...
void A32EmitLoongArch64::EmitA32CallSupervisor(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32ExceptionRaised(A32EmitContext& ctx, IR::Inst* inst) {
    code.SwitchMxcsrOnExit();
...
void A32EmitLoongArch64::EmitA32GetFpscr(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32SetFpscr(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    ctx.reg_alloc.HostCall(nullptr, args[0]);
...
void A32EmitLoongArch64::EmitA32SetFpscrNZCV(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32CoprocInternalOperation(A32EmitContext& ctx, IR::Inst* inst) {
...
void A32EmitLoongArch64::EmitA32CoprocSendOneWord(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
    const auto coproc_info = inst->GetArg(0).GetCoprocInfo();
    const size_t coproc_num = coproc_info[0];
    const bool two = coproc_info[1] != 0;
...
void A32EmitLoongArch64::EmitA32CoprocSendTwoWords(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32CoprocGetOneWord(A32EmitContext& ctx, IR::Inst* inst) {
    const auto coproc_info = inst->GetArg(0).GetCoprocInfo();
...
void A32EmitLoongArch64::EmitA32CoprocGetTwoWords(A32EmitContext& ctx, IR::Inst* inst) {
    const auto coproc_info = inst->GetArg(0).GetCoprocInfo();
    const size_t coproc_num = coproc_info[0];
    const bool two = coproc_info[1] != 0;
...
void A32EmitLoongArch64::EmitA32CoprocLoadWords(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
void A32EmitLoongArch64::EmitA32CoprocStoreWords(A32EmitContext& ctx, IR::Inst* inst) {
    auto args = ctx.reg_alloc.GetArgumentInfo(inst);
...
std::string A32EmitLoongArch64::LocationDescriptorToFriendlyName(const IR::LocationDescriptor& ir_descriptor) const {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::Interpret terminal, IR::LocationDescriptor initial_location, bool) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::ReturnToDispatch, IR::LocationDescriptor, bool) {
    code.ReturnFromRunCode();
}

void A32EmitLoongArch64::EmitSetUpperLocationDescriptor(IR::LocationDescriptor new_location, IR::LocationDescriptor old_location) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::LinkBlock terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::LinkBlockFast terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
    EmitSetUpperLocationDescriptor(terminal.next, initial_location);
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::PopRSBHint, IR::LocationDescriptor, bool is_single_step) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::FastDispatchHint, IR::LocationDescriptor, bool is_single_step) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::If terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::CheckBit terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
    Xbyak::Label fail;
...
void A32EmitLoongArch64::EmitTerminalImpl(IR::Term::CheckHalt terminal, IR::LocationDescriptor initial_location, bool is_single_step) {
    code.cmp(dword[r15 + offsetof(A32JitState, halt_reason)], 0);
    code.jne(code.GetForceReturnFromRunCodeAddress());
    EmitTerminal(terminal.else_, initial_location, is_single_step);
}

void A32EmitLoongArch64::EmitPatchJg(const IR::LocationDescriptor& target_desc, CodePtr target_code_ptr) {
...
void A32EmitLoongArch64::EmitPatchJz(const IR::LocationDescriptor& target_desc, CodePtr target_code_ptr) {
    const CodePtr patch_location = code.getCurr();
    if (target_code_ptr) {
...
void A32EmitLoongArch64::EmitPatchJmp(const IR::LocationDescriptor& target_desc, CodePtr target_code_ptr) {
    const CodePtr patch_location = code.getCurr();
    if (target_code_ptr) {
...
void A32EmitLoongArch64::EmitPatchMovRcx(CodePtr target_code_ptr) {
    if (!target_code_ptr) {
...
void A32EmitLoongArch64::Unpatch(const IR::LocationDescriptor& location) {
    EmitLoongArch64::Unpatch(location);
    if (conf.HasOptimization(OptimizationFlag::FastDispatch)) {
        code.DisableWriting();
