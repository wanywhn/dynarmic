...
namespace Dynarmic::Backend::LoongArch64 {
    ...
    public:
        static constexpr HostLoc::Kind kind = !std::is_same_v<FlagsTag, T>
                                              ? std::is_base_of_v<Xbyak_loongarch64::VReg, T>
                                                ? HostLoc::Kind::Fpr
                                                : HostLoc::Kind::Gpr
                                              : HostLoc::Kind::Flags;
        ...
        RAReg(RAReg &&other)
                : reg_alloc{other.reg_alloc}, rw{std::exchange(other.rw, RWType::Void)},
                  ...
                  write_value{std::exchange(other.write_value, nullptr)}, reg{std::exchange(other.reg, std::nullopt)} {
        }
    ...
    public:
        ...
        explicit RegAlloc(BlockOfCode &code, FpsrManager &fpsr_manager, std::vector<int> gpr_order,
        ...
        auto AllocScratchGpr() {
            auto tmp = RAReg<Xbyak_loongarch64::XReg>{*this, RWType::Read, {}, nullptr,};
            tmp.reg = Xbyak_loongarch64::XReg{getScratchGpr()};
            return tmp;
        }

        auto AllocScratchFpr() {
            auto tmp = RAReg<Xbyak_loongarch64::VReg>{*this, RWType::Read, {}, nullptr,};
            tmp.reg = Xbyak_loongarch64::VReg{getScratchFpr()};
            return tmp;
        }
        ...
        auto ReadWriteX(Argument &arg, const IR::Inst *inst) {
            return RAReg<Xbyak_loongarch64::XReg>{*this, RWType::ReadWrite, arg.value, inst};
        }

        auto ReadWriteW(Argument &arg, const IR::Inst *inst) {
            return RAReg<Xbyak_loongarch64::WReg>{*this, RWType::ReadWrite, arg.value, inst};
        }

        auto ReadWriteQ(Argument &arg, const IR::Inst *inst) {
            return RAReg<Xbyak_loongarch64::VReg>{*this, RWType::ReadWrite, arg.value, inst};
        }
        ...
        void DefineAsRegister(IR::Inst *inst, Xbyak_loongarch64::Reg reg);
    ...
    private:
        ...
        Xbyak_loongarch64::XReg getScratchGpr();

        Xbyak_loongarch64::VReg getScratchFpr();
    };
    ...
    RAReg<T>::RAReg(RegAlloc &reg_alloc, RWType rw, const IR::Value &read_value, const IR::Inst *write_value)
        ...
        // FIXME this could be issue when direct read from inst if reg not enough? let me test an check later.
    ...
    RAReg<T>::~RAReg() {
        ...
        if (reg) {
            reg_alloc.ValueInfo(HostLoc{kind, reg->getIdx()}).realized = false;
        }
    }
